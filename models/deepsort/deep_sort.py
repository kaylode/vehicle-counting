...
class DeepSort(object):
    ...
    def update(self, bbox_xyxy, confidences, ori_img, labels=None):
        self.height, self.width = ori_img.shape[:2]
        # generate detections
        bbox_xywh = self._xyxy_to_xywh(bbox_xyxy)
        features = self._get_features(bbox_xywh, ori_img)
        bbox_tlwh = self._xywh_to_tlwh(bbox_xywh)
        detections = [Detection(bbox_tlwh[i], conf, features[i], label) for i,(conf,label) in enumerate(zip(confidences,labels)) if conf>self.min_confidence]
